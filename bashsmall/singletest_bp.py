...
for time1 in range(5):
    ...
    for time2 in range(6):
        itag = random.randint(5,15)
        for time3 in range(5):
            i = random.randint(3,10)
            j = random.randint(i-1,i+2)
            k = random.randint(j-1,j+2)
            h = random.randint(k-1,k+2)
            tag = str(b)+'_'+str(itag)+'_'+str(i)+'_'+str(j)+'_'+str(k)+'_'+str(h)+'_512_ht'
            tagTableTagWidths = '0 '+str(itag)+' '+str(itag)+' '+str(itag)+' '+str(itag)
            # tagTableTagWidths = '0 7 7 8 8'
            logTagTableSizes = str(b)+' '+str(i)+' '+str(j)+' '+str(k)+' '+str(h)
            os.environ["BNUM"]=tag
