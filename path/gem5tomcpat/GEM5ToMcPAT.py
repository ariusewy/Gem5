...
def main():
    ...
    #下面这一段是我加的，可以自动生成mcpat.log，不会覆盖已经生成的，要生成新的需要把旧的删掉或者改名字，有需求的自己改改就好，也比较简单
    for root, dirs, files in os.walk("../out"):
        if 'config.json' in files and 'stats.txt' in files and outname not in files and 'not.txt' not in files:
            opts = {"verbose": True, "out": root + '/' + outname}
            args = [root+'/stats.txt',root + '/config.json', 'boom-template.xml']
            print(root)
            runoa(opts,args)
            print(root)
    for root, dirs, files in os.walk("../out"):
        if outname in files and 'mcpat.log' not in files:
            opts = {"verbose": True, "out": root + '/' + outname}
            command = "mcpat -infile  " + opts["out"] + " -print_level 5 >  "+ root + "/mcpat.log"
            os.system(command)
            #
def runoa(opts,args):

    # args = ['../out/bsrandinst2000unit16delay/stats.txt', '../out/bsrandinst2000unit16delay/config.json', 'template-xeon.xml']
...
def readStatsFile(statsFile):
    ...
    statLine = re.compile(r'([a-zA-Z0-9_\.:-]+)\s+([-+]?[0-9]+\.[0-9]+|[-+]?[0-9]+| nan|inf)') # get like: system.cpu.numCycles   930890554
    count = 0 
    for line in F:
        ...
        if not ignores.match(line):
            ...
            if statValue == 'nan':
                ...
                statValue = '0'
            stats[statKind] = statValue #Get dict ["system.cpu.numCycles",930890554]
    F.close()
